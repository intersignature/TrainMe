...
//        self.progressTableView.reloadData()
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        statusSegmented.setTitle("confirm_progress".localized(), forSegmentAt: 0)
        statusSegmented.setTitle("payment_progress".localized(), forSegmentAt: 1)
        statusSegmented.setTitle("ongoing_progress".localized(), forSegmentAt: 2)
        statusSegmented.setTitle("successful_progress".localized(), forSegmentAt: 3)
    ...
    func getPendingDataList() {
        ...
        self.ref.child("pending_schedule_detail").child(self.currentUser.uid).observe(.value, with: { (snapshot) in
            ...
            guard let value = snapshot.value as? [String: [String: NSDictionary]] else { return }
            value.forEach({ (schedule_key, pendingDataObj) in
                pendingDataObj.forEach({ (pendingDataObjKey, pendingDataObjVal) in
                    let pendingData = PendingBookPlaceDetail()
                    pendingData.schedule_key = schedule_key
            ...
            self.courseIds.forEach({ (eachCourseId) in
                if self.courseObj[eachCourseId] == nil {
                    self.checkSortDate = false
                    self.getCourseData(courseId: eachCourseId)
                }
            })
            if !isSortDate {
                self.sortDate()
...
//            self.finishTask = false
            ...
            guard let values = snapshot.value as? [String: [String: AnyObject]] else { return }
    ...
    func getTraineeData(uid: String) {
        ...
        self.ref.child("user").child(uid).observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            if self.traineeObj.count == self.traineeIds.count && self.traineeObj.count != 0 &&
                ...
                self.placeObj.count == self.placeIds.count && self.placeObj.count != 0 && !self.checkSortDate {
                self.sortDate()
            }
        }) { (err) in
            print(err.localizedDescription)
...
//            cell.traineeNameLb.accessibilityLabel = self.traineeObj[self.pendingDataListsMatch[indexPath.section].pendingDetail[indexPath.row].trainee_id]!.uid
            ...
            cell.declineBtn.setTitle("decline".localized(), for: .normal)
...
//            dataToTraineeProfile.append(tapLabel.accessibilityElements![1] as! Bool)
    ...
    @objc func acceptBtnAction(acceptBtn: UIButton) {
        ...
        let alert = UIAlertController(title: "confirm_to_accept".localized(), message: "", preferredStyle: UIAlertController.Style.alert)
...
//                    }
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        case 1:
            let label = UILabel()
            label.text = "wait_your_trainee_for_pay".localized()
            label.font = UIFont.boldSystemFont(ofSize: 14.0)
            label.numberOfLines = 1
            label.translatesAutoresizingMaskIntoConstraints = false
